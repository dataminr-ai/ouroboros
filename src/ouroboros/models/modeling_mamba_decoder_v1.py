...
class MambaMixer(nn.Module):
    ...
    def janky_forward(self, input_states, cache_params: Optional[MambaCache]=None):
        batch_size, seq_len, _ = input_states.shape
        dtype = input_states.dtype
        # 1. Gated MLP's linear projection
        projected_states = self.in_proj(input_states).transpose(1, 2)                   # [batch, 2 * intermediate_size, seq_len]
    ...
    def forward(self, hidden_states, cache_params: Optional[MambaCache] = None, forward_type="slow-inference"):
        if forward_type == "slow-inference":
            return self.slow_forward(hidden_states, cache_params=cache_params)
        elif forward_type == "slow-training":
            return self.janky_forward(hidden_states, cache_params=cache_params)
...
class MambaBlock(nn.Module):
    ...
    def forward(self, hidden_states, cache_params: Optional[MambaCache] = None, forward_type="slow-inference"):
        ...
        hidden_states = self.mixer(hidden_states, cache_params=cache_params, forward_type=forward_type)
        ...
        **kwargs,  # `attention_mask` is passed by the tokenizer and we don't want it
        ...
        for mixer_block in self.layers:
            if self.gradient_checkpointing and self.training:
                ...
                hidden_states = mixer_block(hidden_states, cache_params=cache_params, forward_type=forward_type)
